...
class SSDP:
    ...
    def __init__(self):
        ...
        print("ssdp constructor -> ssdp.py")
        self.entries = []  # type: List[UPNPEntry]
        print("SSDP entries: ", self.entries)
        self.last_scan = None

    def scan(self):
        """Scan the network."""
        print("test ssdp.scan")
        self.update()
...
class UPNPEntry:
    ...
    def description(self):
        ...
        print("description: " , UPNPEntry.DESCRIPTION_CACHE[url])
    ...
    def match_device_description(self, values):
        ...
        if device is None:
            return False
        print("match description: ", all(device.get(key) in val
...
def scan(timeout=DISCOVER_TIMEOUT):
    ...
    try:
        ...
        for s in sockets:
            s.close()
    print("UPNP scan results: ",entries)
    print("Sorted UPNP entries: ", sorted(entries.values(), key=lambda entry: entry.location or ''))
